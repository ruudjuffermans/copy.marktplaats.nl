    ...
    <CardBlock>
      <AccountNav>
        {page === "login" ? (
          ...
          <AccountTab href={"/account/login"}>Inloggen</AccountTab>
        )}

        {page === "register" ? (
          <AccountTab active>Account Aanmaken</AccountTab>
        ) : (
          <AccountTab href={"/account/register"}>Account Aanmaken</AccountTab>
        )}
      </AccountNav>
    </CardBlock>
  );
...
const AccountLayout = () => {
  ...
  return (
    <PageWrapper>
      <PageContent mw={pathname === "/reset" ? "600px" : "800px"}>
        <PageHeadingH1>{pathname}</PageHeadingH1>
        <PageCard>
          <Outlet />
        </PageCard>
        {pathname === "/login" && (
